...
def getid(img_id, cookies):
    url = 'http://www.pixiv.net/member_illust.php?id=' + str(img_id)
    dataids = []
    empty = False
    p = 1
    while not empty:
...
def getid2(img_id, cookies):
    url = 'http://www.pixiv.net/bookmark.php?id=' + str(img_id)
    dataids = []
    empty = False
    p = 1
    while not empty:
